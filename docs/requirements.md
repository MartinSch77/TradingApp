...
| REQ-F-012 | The trade panel shall default the invest amount to 3750 (display currency) and auto-propose SL/TP from recent volatility (stop ≈ 1.5σ of the horizon move, TP = 1.5 × SL) until the user edits them; an instrument switch re-enables the automatics. The automatic proposal shall never overwrite a stop-loss or take-profit field while it is being edited (keyboard focus inside the field). | T/I |
