...
| linuxdeploy (AppImage) | **windeployqt** | different shape, same job: the portable ZIP (`tools\package_portable.ps1`) is produced by `cmake --install`, which runs windeployqt through the install rules — Qt DLLs, `platforms\qwindows.dll`, the Schannel TLS backend, styles and the compiler runtime. The script fails if `platforms\qwindows.dll` is missing, because that is the one omission a user only discovers as "no Qt platform plugin could be initialized". |
